...
新的 Mac DMG 完成 Developer ID 签名、公证并公开到 GitHub Releases 后，先将主仓库 README 的 Mac 下载按钮链接更新为新版 DMG 的直接下载地址，再同步更新 tap 中 `Casks/tower.rb` 的版本、构建号与该文件的 SHA-256。执行 `brew style --cask pengchujin/tap/tower`、`brew audit --cask pengchujin/tap/tower` 并验证安装后推送；仅发布 GitHub Release 不会自动更新 cask。 后续更新只改版本、构建号和 SHA-256，保留当前 cask 语法：URL 不加已弃用的 `verified:`，最低系统写作 `depends_on macos: :sonoma`（仍表示 macOS 14 及以上）；不要从旧模板恢复弃用写法。cask 的 homepage 保持官方 GitHub 仓库，与下载域名一致，兼容仍要求跨域 verified 的旧版 Homebrew。用户运行 `brew update` 和 `brew upgrade --cask pengchujin/tap/tower` 获取新包。
